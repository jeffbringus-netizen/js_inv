import { $, esc, copyToClipboard } from './ui.mjs';
...
const webstockCsvInput = $('#webstockCsvInput');
$('#webstockUploadZone').addEventListener('click', () => webstockCsvInput.click());
// keep the input's programmatic click from bubbling back to the zone (re-click loop cancels the chooser)
webstockCsvInput.addEventListener('click', e => e.stopPropagation());
