...
export function paginationHtml(meta, target, label) {
  ...
  if (pages <= 1) return '';
  const archivedLabel = meta.archivedTotal > 0 ? ` (${meta.archivedTotal.toLocaleString()} archived)` : '';
  return `<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 py-2">
    <span class="text-muted small">${meta.total.toLocaleString()} ${label}${archivedLabel}</span>
